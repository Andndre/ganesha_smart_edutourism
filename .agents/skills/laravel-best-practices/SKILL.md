...
- For Laravel Dusk browser tests, use `DatabaseMigrations` or `DatabaseTruncation` instead of `RefreshDatabase`/`LazilyRefreshDatabase`
- Use `dusk` selectors (`@element`) and waiting methods (`waitForText`) for resilient Dusk testing

...
<div class="mb-8">
    ...
    <p class="mt-1 text-sm text-gray-500">Tentukan letak geografis toko Anda di peta desa wisata Penglipuran agar wisatawan dapat mencari dan bernavigasi ke toko Anda secara real-time.</p>
</div>
    ...
    <div class="rounded-2xl border border-warning/20 bg-warning/5 p-6 shadow-sm max-w-3xl">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-warning-700">Silakan isi profil toko Anda terlebih dahulu sebelum mengkustomisasi lokasi toko di peta.</p>
                <div class="mt-4">
    ...
    <div class="grid gap-8 lg:grid-cols-3 max-w-6xl">
        ...
        <div class="lg:col-span-2 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            <div class="mb-3 flex items-center justify-between px-2">
                ...
                <span class="text-xs font-medium text-primary">Klik di mana saja pada peta atau geser pin untuk memperbarui lokasi</span>
            </div>
            ...
            <form method="POST" action="{{ route('owner.location.update') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="latitude" id="field-lat" readonly required value="{{ old('latitude', $location->latitude ?? '') }}" class="mt-1 w-full rounded-xl bg-gray-50 border border-gray-200 px-4 py-2.5 font-mono text-sm text-gray-500 focus:outline-none">
                    </div>

                    {{-- Longitude --}}
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-gray-400">Longitude</label>
                        <input type="text" name="longitude" id="field-lng" readonly required value="{{ old('longitude', $location->longitude ?? '') }}" class="mt-1 w-full rounded-xl bg-gray-50 border border-gray-200 px-4 py-2.5 font-mono text-sm text-gray-500 focus:outline-none">
                    </div>
                    ...
                    <div class="flex items-start gap-3 mt-4">
                        <input type="checkbox" name="is_accessible" id="field-accessible" value="1" {{ old('is_accessible', $location->is_accessible ?? true) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary">
                        <div>
                            <label for="field-accessible" class="text-sm font-semibold text-gray-700">Akses Ramah Disabilitas</label>
                            <p class="text-xs text-gray-400">Centang jika toko Anda memiliki akses ramah pengguna kursi roda atau lansia.</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="accessibility_notes" rows="3" placeholder="Contoh: Tersedia ramp landai di pintu masuk utama." class="mt-1.5 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20">{{ old('accessibility_notes', $location->accessibility_notes ?? '') }}</textarea>
                    </div>
                </div>

                <div class="mt-6 border-t border-gray-100 pt-4">
                    <button type="submit" class="w-full inline-flex justify-center items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-primary/20 transition-all hover:bg-primary-600 active:scale-[0.98]">

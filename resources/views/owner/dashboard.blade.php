    ...
    <div class="rounded-2xl border border-warning/20 bg-warning/5 p-6 shadow-sm max-w-3xl">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-warning-700">Akun Anda belum memiliki profil toko UMKM yang aktif. Silakan isi informasi profil toko Anda terlebih dahulu agar dapat mengelola produk dan menandai lokasi di peta.</p>
                <div class="mt-4">
...
@else
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mb-8">
        ...
        <div class="relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
        ...
        <div class="relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 font-display text-3xl font-bold text-charcoal">{{ number_format($profile->rating ?? 5.0, 1) }}</p>
                </div>
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-lg bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">
                ...
                <div>
                    ...
                    @if ($profile->is_active)
                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-secondary/10 px-2.5 py-1 text-xs font-bold text-secondary">Buka / Aktif</span>
                    @else
                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-gray-150 px-2.5 py-1 text-xs font-bold text-gray-500">Tutup / Nonaktif</span>
                    @endif
                </div>
            </div>
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 leading-relaxed">{{ $profile->description ?? 'Belum ada deskripsi.' }}</p>
                </div>
                <div>
                    <span class="text-xs text-gray-400 block font-semibold uppercase">Lokasi Koordinat Peta</span>
                    @if ($profile->mapLocation)
                        <span class="text-sm font-mono text-gray-600 block">Lat: {{ $profile->mapLocation->latitude }}, Lng: {{ $profile->mapLocation->longitude }}</span>
                        <a href="{{ route('owner.location') }}" class="mt-1 inline-flex items-center gap-1 text-xs font-bold text-primary hover:underline">
                            Kustomisasi di Peta →
                        </a>
                    @else
                        <span class="text-sm italic text-warning font-semibold block">Lokasi belum diatur di peta!</span>
                        <a href="{{ route('owner.location') }}" class="mt-1 inline-flex items-center gap-1 text-xs font-bold text-warning hover:underline">

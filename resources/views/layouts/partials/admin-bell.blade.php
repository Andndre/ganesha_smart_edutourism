...
<div x-data="adminBell()" class="relative">
    <button type="button" @click="toggle()"
        ...
        <span x-show="unread > 0" x-cloak x-text="unread > 9 ? '9+' : unread"
            class="absolute -top-0.5 -right-0.5 min-w-[18px] rounded-full bg-warning px-1 text-[10px] font-bold leading-[18px] text-white text-center"></span>
    </button>

    <div x-show="open" x-cloak @click.outside="open = false" @keydown.escape.window="open = false"
        x-transition.origin.top.right
        class="absolute right-0 mt-2 w-80 max-h-[70vh] overflow-y-auto rounded-2xl border border-gray-100 bg-white shadow-xl z-50">

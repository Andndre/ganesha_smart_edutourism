<script>
    ...
    .preventDefault(); // Must be synchronous before any await — async return value is a Promise (truthy), so onsubmit="return ..." cannot prevent submit
        ...
        try {
            ...
            if (formData.get('tmp_model_3d_path')) {
                formData.delete('model_3d_file');
            }
            if (formData.get('tmp_model_3d_usdz_path')) {
                formData.delete('model_3d_usdz_file');
            }
            ...
            if (data.success && data.model) {
                ...
                closeModelModal();
                window.dispatchEvent(new CustomEvent('ar-model-created', {
                    detail: {
                        model: data.model
                    }
                }));
    ...
    (function() {
        ...
        initChunkedUpload('model-field-usdz-file', 'model-field-tmp-usdz', 'model-usdz-progress', 50 * 1024 * 1024,
            ['.usdz']);
    })();
</script>

    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @forelse ($categories as $cat)
            <div @if ($loop->first) id="tour-first-card" @endif
                class="flex flex-col justify-between rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:shadow-md">
                <div>
                ...
                <div>
                    ...
                    <div @if ($loop->first) id="tour-actions" @endif class="mt-4 flex gap-2">
                        <button
                            ...
                            class="flex-1 rounded-xl border border-gray-200 py-2 text-center text-xs font-semibold text-gray-600 transition-colors hover:bg-gray-50">
                            Edit
                        </button>
                        <form method="POST" action="{{ route('admin.umkm.categories.destroy', $cat->id) }}"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="border-warning/30 text-warning hover:bg-warning/5 w-full rounded-xl border py-2 text-xs font-semibold transition-colors">
                                Hapus
                            </button>
                        </form>
                    </div>
        ...
        <form id="modal-form" method="POST" action="" enctype="multipart/form-data" x-data="{ locale: 'en' }">
            ...
            <div class="space-y-4">
                ...
                <div x-show="locale === 'en'">
                    ...
                    <textarea name="description[en]" id="field-description-en" placeholder="Short description in English..."
                        rows="3"
                ...
                <div x-show="locale === 'id'">
                    ...
                    <textarea name="description[id]" id="field-description-id" placeholder="Deskripsi singkat tentang kategori..."
                        rows="3"
                        class="focus:border-primary mt-1 w-full resize-none rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:outline-none"></textarea>
...
@push('scripts')
    <script>
        ...
        function openEditModal(cat) {
            ...
            fieldDescriptionEn.value = (typeof cat.description === 'object') ? (cat.description?.en || "") : (cat
                .description || "");

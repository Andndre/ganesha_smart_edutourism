            ...
            @if (session('success'))
                ...
                @if (session('guestUrl'))
                    <div class="mb-6 rounded-xl border border-indigo-200 bg-indigo-50 p-6 text-center shadow-sm">
                        ...
                        <p class="mt-2 text-sm text-indigo-700">Silakan arahkan pengunjung untuk memindai QR Code di
                            bawah ini untuk mengakses Smart Edutourism tanpa perlu mendaftar akun.</p>
                        <div class="mt-4 flex justify-center">
                            <div class="rounded-lg bg-white p-4 shadow-sm" id="qrcode"></div>
                        </div>
                        <div class="mt-4">
                            <p class="text-xs text-indigo-500">Atau akses URL berikut: <br><a
                                    href="{{ session('guestUrl') }}" class="font-medium underline"
                                    target="_blank">{{ session('guestUrl') }}</a></p>
                        </div>
                ...
                <div class="col-span-1 rounded-xl bg-white p-6 shadow-sm">
                    ...
                    <form action="{{ route('staff.ticketing.walk-in') }}" method="POST">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label for="tour_package_id" class="block text-sm font-medium text-gray-700">Pilih Paket
                                    Wisata</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label for="party_size" class="block text-sm font-medium text-gray-700">Jumlah
                                        Orang</label>
                                    <input type="number" name="party_size" id="party_size" min="1"
                                        value="1" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                </div>
                                <div>
                                ...
                                @forelse($reservations as $res)
                                    <tr>
                                        <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">
                                            ...
                                            {!! $res->user
                                                ? ''
                                                : '<span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800">Walk-in</span>' !!}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            ...
                                            <span class="text-xs">Rp
                                                {{ number_format($res->total_amount, 0, ',', '.') }}</span>
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                            @if ($res->status === 'completed')
                                                <span
                                                    class="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">Selesai/Masuk</span>
                                            @elseif($res->status === 'confirmed')
                                                <span
                                                    class="inline-flex rounded-full bg-yellow-100 px-2 text-xs font-semibold leading-5 text-yellow-800">Menunggu</span>
                                            @else
                                                <span
                                                    class="inline-flex rounded-full bg-gray-100 px-2 text-xs font-semibold leading-5 text-gray-800">{{ ucfirst($res->status) }}</span>
                                            @endif
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">

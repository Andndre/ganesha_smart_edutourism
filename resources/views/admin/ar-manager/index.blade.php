...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-3 flex flex-col shadow-2xs hover:shadow-xs transition-shadow">
            ...
            @if ($m->ar_marker_id)
                <div class="mt-2 flex items-center gap-1.5">
                    <span class="font-mono text-[10px] bg-primary/10 text-primary font-bold px-2 py-0.5 rounded-full truncate max-w-[160px]">{{ $m->ar_marker_id }}</span>
                    <button type="button"
                        onclick="triggerMarkerDownload('{{ $m->ar_marker_id }}', '{{ $m->mapLocation?->locationable?->slug ?? '' }}')"
                        class="ml-auto shrink-0 rounded-lg p-1.5 text-primary hover:bg-primary/10 transition-colors"
            ...
            @if ($m->mapLocation)
                <span class="mt-1 text-[10px] text-gray-500 font-medium truncate">📍 {{ $m->mapLocation->name }}</span>
            @endif

            <div class="mt-2.5 pt-2 border-t border-gray-50 flex items-center justify-end gap-1">
                ...
                <form method="POST" action="{{ route('admin.ar-manager.models.destroy', $m->id) }}" class="delete-form inline" data-confirm="Apakah Anda yakin ingin menghapus model ini?">
                    @csrf
                    @method('DELETE')
    ...
    @empty
        <div class="col-span-3 py-12 text-center text-gray-400 text-sm">Belum ada model 3D ditambahkan.</div>
    @endforelse
</div>
    ...
    <form id="model-form" method="POST" action="" enctype="multipart/form-data">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <span class="block text-[10px] text-gray-400 mb-1.5">Opsional. Harus unik. Digunakan sebagai kode referensi marker fisik.</span>
                <input type="text" name="ar_marker_id" id="model-field-marker-id" placeholder="Contoh: MARKER_PURA_01"
                    oninput="generateARMarkerInModal()"
                ...
                <div id="modal-marker-preview-wrapper" class="mt-2 hidden">
                    <div class="flex items-center gap-3 rounded-xl border border-dashed border-gray-200 bg-gray-50/50 p-3">
                        <div id="modal-marker-canvas-placeholder" class="bg-white border border-gray-100 rounded-lg p-1 shadow-xs shrink-0"></div>
                        <div class="flex flex-col gap-1.5 min-w-0">
                            ...
                            <button type="button" onclick="downloadARMarkerFromModal()"
                                class="border-primary text-primary hover:bg-primary/5 flex items-center justify-center gap-1.5 rounded-lg border py-1.5 px-3 text-xs font-semibold transition-colors">
            ...
            <div>
                ...
                <div id="model-desc-toolbar"
                    ...
                    <button type="button" data-action="bold"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Bold">
                    ...
                    <button type="button" data-action="italic"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Italic">
                    ...
                    <button type="button" data-action="bulletList"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Bullet List">
                    ...
                    <button type="button" data-action="orderedList"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Ordered List">
                    ...
                    <button type="button" data-action="undo"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Undo">
                    ...
                    <button type="button" data-action="redo"
                        class="flex items-center justify-center rounded-lg border border-transparent p-1.5 transition-colors hover:bg-gray-200/70 hover:text-gray-900" title="Redo">
                ...
                <div id="model-desc-editor"
                    class="focus-within:border-primary focus-within:ring-primary/20 max-h-[200px] min-h-[100px] w-full overflow-y-auto rounded-b-xl border border-gray-200 bg-white p-4 text-sm focus-within:ring-1">
                </div>
...
<script>
    ...
    function openModelModal() {
        ...
        if (window.modelDescEditor) { window.modelDescEditor.commands.setContent(''); }
    ...
    function openModelEditModal(model) {
        ...
        if (window.modelDescEditor) { window.modelDescEditor.commands.setContent(descHtml); }
        document.getElementById('model-field-glb-file').value = "";
        ...
        if (model.audio_narration_path) {
            document.getElementById('edit-current-audio-path').textContent = model.audio_narration_path.split('/').pop();
    ...
    function generatePattText(canvas, borderWidth, patternSize) {
        ...
        for (let r = 0; r < gridSize; r++) {
            ...
            for (let c = 0; c < gridSize; c++) {
                ...
                grid[r][c] = `${(sumR/count/255).toFixed(3)} ${(sumG/count/255).toFixed(3)} ${(sumB/count/255).toFixed(3)}`;
            }
        }

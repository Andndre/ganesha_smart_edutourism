    ...
    @forelse ($feedbacks as $f)
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            ...
            <div class="mt-3 flex gap-2">
                @if (!$f->admin_response)
                    <button onclick="toggleReplyForm({{ $f->id }})" class="rounded-lg border border-primary/20 px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary/5 transition-colors">
                        Balas
                    </button>
                @endif
                ...
                <form action="{{ route('admin.feedback.toggle', $f->id) }}" method="POST" class="inline">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-500 hover:bg-gray-50 transition-colors">
                        {{ $f->is_public ? 'Sembunyikan Publik' : 'Tampilkan Publik' }}
                    </button>
                </form>

                <form action="{{ route('admin.feedback.destroy', $f->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ulasan ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="rounded-lg border border-warning/20 px-3 py-1.5 text-xs font-semibold text-warning hover:bg-warning/5 transition-colors">
                        Hapus
                    </button>
                </form>
            </div>
            ...
            <div id="reply-form-{{ $f->id }}" class="mt-4 hidden bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                <form action="{{ route('admin.feedback.reply', $f->id) }}" method="POST">
                    @csrf
                    <label for="admin-response-input-{{ $f->id }}" class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1.5">Tulis Balasan Admin</label>
                    <textarea name="admin_response" id="admin-response-input-{{ $f->id }}" rows="2" required class="w-full rounded-xl border border-gray-200 p-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary bg-white text-charcoal mb-3" placeholder="Tulis balasan Anda di sini..."></textarea>
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="toggleReplyForm({{ $f->id }})" class="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-500 hover:bg-gray-50">
                            Batal
                        </button>
                        <button type="submit" class="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-white shadow-md shadow-primary/20 hover:bg-primary-dark">
                            Kirim Balasan
                        </button>
                    </div>
                </form>

...
@section('title', 'Kelola Petugas Tiket')

@section('content')

<div class="mb-6 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="font-display text-2xl font-bold text-charcoal">Akun Petugas Tiket</h1>
        <p class="mt-0.5 text-sm text-gray-500">Kelola kredensial akun petugas tiket untuk pemindaian tiket masuk dan layanan walk-in.</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 bg-gray-50/50">
                    <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Nama Petugas</th>
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Tanggal Terdaftar</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($officers as $officer)
                    <tr class="hover:bg-gray-50/50">
                        <td class="px-5 py-4 font-semibold text-charcoal">{{ $officer->name }}</td>
                        <td class="px-5 py-4 text-gray-500 font-mono text-xs">{{ $officer->email }}</td>
                        <td class="px-5 py-4 text-gray-600">{{ $officer->phone ?? '-' }}</td>
                        <td class="px-5 py-4 text-gray-500 text-xs">{{ $officer->created_at ? $officer->created_at->translatedFormat('d M Y H:i') : '-' }}</td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-2">
                                <button onclick="openEditModal({{ json_encode($officer) }})" class="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-primary/10 hover:text-primary" title="Edit">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <form method="POST" action="{{ route('admin.ticket-officers.destroy', $officer->id) }}" class="delete-form inline" data-confirm="Apakah Anda yakin ingin menghapus akun petugas tiket ini?">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-gray-400">Belum ada akun petugas tiket.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
{{-- Officer Modal Form --}}
<div id="officer-modal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-charcoal/50 backdrop-blur-sm p-4 justify-center">
    <div class="my-auto self-start w-full max-w-md rounded-2xl bg-white p-6 shadow-xl transition-all">
        <div class="mb-4 flex items-center justify-between">
            <h3 id="modal-title" class="font-display text-lg font-bold text-charcoal">Buat Akun Petugas Tiket</h3>
        ...
        <form id="modal-form" method="POST" action="">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700">Nama Petugas <span class="text-warning">*</span></label>
                    <input type="text" name="name" id="field-name" required placeholder="Contoh: I Kadek Wira" class="mt-1 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-primary focus:outline-none">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700">Email Login <span class="text-warning">*</span></label>
                    <input type="email" name="email" id="field-email" required placeholder="Contoh: kadek@example.com" class="mt-1 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-primary focus:outline-none">
                </div>
                <div>
...
<script>
    ...
    function openEditModal(officer) {
        ...
        fieldPhone.value = officer.phone || "";
        fieldPassword.value = "";

...
<div x-data="{ isOpen: {{ $defaultOpen ? 'true' : 'false' }} }" x-show="isOpen" @open-{{ $name }}.window="isOpen = true"
    ...
    <div class="{{ $desktopContainerClass }} {{ $maxWidthClass }} pointer-events-auto relative flex w-full flex-col rounded-t-[2.5rem] bg-white p-6 pb-10 shadow-2xl md:pb-6"
        style="padding-bottom: calc(1.5rem + env(safe-area-inset-bottom));" @click.outside="!document.querySelector('.swal2-container') && (isOpen = false)" x-show="isOpen"
        ...
        <button type="button" @click="isOpen = false"
            class="absolute right-4 top-4 hidden h-8 w-8 items-center justify-center rounded-full bg-gray-50 text-gray-400 transition-colors hover:text-gray-600 md:flex"
            title="Tutup">

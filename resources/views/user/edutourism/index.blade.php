        ...
        @forelse($routes as $route)
            <div class="flex flex-col justify-between rounded-3xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:shadow-md">
                    ...
                    <div class="space-y-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if($completedRouteIds->contains($route->id))
                                <span class="flex items-center gap-1 rounded-lg bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-600">
                ...
                <button @click="@auth $dispatch('open-route-preview-modal'); fetchRoutePreview({{ $route->id }}) @else $dispatch('open-save-progress-confirm-modal'); window.pendingRouteId = {{ $route->id }} @endauth"
                    class="{{ $completedRouteIds->contains($route->id) ? 'bg-white border-2 border-[#1E5128] text-[#1E5128] hover:bg-gray-50' : 'bg-[#1E5128] hover:bg-[#152E1D] text-white' }} mt-5 block w-full rounded-2xl py-3 text-center text-sm font-bold transition-transform active:scale-95 shadow-sm">

        ...
        (function() {
            ...
            function updateRouteButtonUI() {
                ...
                if (isGpsLoading) {
                    ...
                    `;
                    textEl.textContent = @js(__('Mencari GPS...'));
                } else {
                    iconEl.innerHTML = `
                    ...
                    `;
                    textEl.textContent = @js(__('Arahkan'));
                }
            }
            ...
            function initMap() {
                ...
                // Listen for search input typing (debounced so the map doesn't jump per keystroke)
                ...
                // Close filter panel, search results, and active user route when clicking elsewhere on map
            ...
            function updateVisibleMarkers() {
                ...
                markerLayers.forEach(item => {
                    ...
                    const matchesSearch = !query ||
                        ...
                        (item.loc.raw_name && item.loc.raw_name.toLowerCase().includes(query)) ||
                ...
                if (query && matches.length > 0) {
                    map.fitBounds(L.latLngBounds(matches.map(loc => [loc.lat, loc.lng])), {
                        padding: [70, 70],
                        maxZoom: 18,
                        animate: true,
                        duration: 0.5
            ...
            function renderSearchResults(query, matches) {
                ...
                // Results dropdown and filter panel occupy the same spot — keep one visible
                ...
                if (matches.length === 0) {
                    ...
                    matches.forEach(loc => {
                        ...
                        dot.style.backgroundColor = categoryColors[loc.cat] || '#1E5128';
                ...
                if (secAcc && sheetAcc) {
                    if (loc.is_accessible) {
                        sheetAcc.textContent = loc.accessibility && loc.accessibility.trim() !== '' ?
                            loc.accessibility :
                            @js(__('Akses ramah disabilitas tersedia.'));

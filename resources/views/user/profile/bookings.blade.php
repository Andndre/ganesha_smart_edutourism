...
@section('content')
    <div class="px-4 py-6" x-data="{
        tab: '{{ $filter }}',
        statuses: {{ $reservations->pluck('status')->toJson() }},
        matches(r) { return this.tab === 'semua' || (this.tab === 'aktif' && r === 'confirmed') || (this.tab === 'selesai' && (r === 'completed' || r === 'cancelled')); },
        get visibleCount() { return this.statuses.filter(s => this.matches(s)).length; }
    }">
        <!-- Status Filter -->
        <div class="no-scrollbar mb-6 flex gap-2 overflow-x-auto">
            @foreach (['semua' => __('Semua'), 'aktif' => __('Aktif'), 'selesai' => __('Selesai')] as $key => $label)
                <button type="button"
                    @click="tab = '{{ $key }}'; history.replaceState(null, '', '{{ route('bookings') }}?filter={{ $key }}')"
                    :class="tab === '{{ $key }}' ? 'bg-primary font-bold text-white border-primary' :
                        'border-gray-200 bg-white font-medium text-gray-500 hover:bg-gray-50'"
                    class="whitespace-nowrap rounded-full border px-4 py-2 text-sm transition-colors">{{ $label }}</button>
            @endforeach
        </div>

        <div class="space-y-4">
            @forelse($reservations as $reservation)
                <div x-show="matches('{{ $reservation->status }}')" x-cloak
                    class="relative flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
                    <!-- Status Badge -->
                    @php
                    ...
                    <div class="p-4">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-y-2 rounded-xl bg-gray-50 p-3 text-sm">
                            ...
                            <div>
                                ...
                                <p class="text-charcoal font-medium">{{ $reservation->party_size }} {{ __('Orang') }}
                                </p>
                            </div>
                        ...
                        @elseif($reservation->status == 'pending')
                            <div class="mt-4 flex gap-2 border-t border-gray-100 pt-4">
                                <button type="button" onclick="payBooking({{ $reservation->id }})"
            ...
            @empty
                <div class="mt-8 rounded-3xl border border-gray-100 bg-white p-8 text-center shadow-sm">
                    ...
                    <p class="mb-6 text-sm text-gray-500">
                        {{ __('Anda belum pernah memesan tiket atau paket wisata apapun.') }}</p>
            ...
            @if ($reservations->count() > 0)
                <div x-show="visibleCount === 0" x-cloak
                    class="mt-8 rounded-3xl border border-gray-100 bg-white p-8 text-center shadow-sm">
                    <div
                        class="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gray-50 text-gray-300">
                        <svg class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    ...
                    <h3 class="text-charcoal mb-2 text-lg font-bold">{{ __('Tidak Ada Pesanan') }}</h3>
                    <p class="text-sm text-gray-500">{{ __('Tidak ada pesanan dengan status ini.') }}</p>
                </div>
            @endif
        </div>
            ...
            try {
                ...
                @if (config('midtrans.is_production'))
                    snap.pay(data.snap_token, {
                        ...
                        onError: function() {
                            alert('{{ __('Pembayaran gagal atau dibatalkan.') }}');
                        },
                        onClose: function() {
                            alert('{{ __('Anda menutup popup pembayaran sebelum menyelesaikannya.') }}');
                        }
                    });
                @else
    ...
    <div x-data="{ qrUrl: '', ticketName: '' }"
        ...
        <x-modal name="ticket-modal" maxWidth="sm">
            <div class="text-center">
                ...
                <p class="mb-6 text-sm text-gray-500">
                    {{ __('Tunjukkan QR Code ini kepada petugas tiket saat Anda tiba di lokasi wisata.') }}</p>

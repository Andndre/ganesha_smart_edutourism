...
<div id="model-view" class="absolute inset-0 z-10 hidden bg-[#1a1a1a]">
    ...
    <div id="iab-warning"
        class="z-60 absolute inset-0 hidden flex-col items-center justify-center bg-black/90 p-6 text-center text-white backdrop-blur-sm">
        ...
        <p class="mb-8 text-base leading-relaxed text-gray-300">
            ...
            <strong class="text-white">{{ __('Buka di Browser Sistem / Safari') }}</strong>.
        </p>
        <button
            onclick="document.getElementById('iab-warning').classList.add('hidden'); document.getElementById('iab-warning').classList.remove('flex');"
            class="rounded-full border border-gray-600 px-6 py-2 text-sm font-medium text-gray-400 transition-colors hover:bg-gray-800 hover:text-white">
        ...
        <div id="ar-error"
            ...
            <div>
                ...
                <p class="mb-6 text-sm text-gray-400">
                    {{ __('Perangkat Anda mungkin tidak mendukung teknologi WebXR atau ARCore.') }}</p>
                <button
                    onclick="const el=document.getElementById('ar-error');el.classList.add('hidden');el.classList.remove('flex')"
    ...
    <div class="pointer-events-none absolute inset-0 z-30 flex flex-col justify-end overflow-hidden">
        ...
        <div id="sheet-backdrop"
            class="pointer-events-auto absolute inset-0 bg-black/40 opacity-0 transition-opacity duration-300"
            style="display: none;"></div>

        <!-- Sheet Container -->
        <div id="desc-bottom-sheet"
            class="rounded-t-4xl pointer-events-auto relative z-10 flex h-[75vh] w-full translate-y-[calc(100%-100px)] flex-col bg-white shadow-[0_-4px_25px_rgba(0,0,0,0.15)] transition-transform duration-300 ease-in-out">
            <!-- Header (Always visible, height: 100px) -->
            <div id="sheet-header"
                class="h-25 rounded-t-4xl flex shrink-0 cursor-pointer flex-col items-center px-6 pt-3 transition-colors active:bg-gray-50">
                <div class="flex w-full justify-center">
                    <svg id="sheet-arrow" class="mb-2 h-6 w-6 text-gray-400 transition-transform duration-300"
            ...
            <div class="flex-1 overflow-y-auto px-6 pb-12 pt-4">
                <div id="model-desc-full" class="prose prose-sm prose-gray max-w-none leading-relaxed text-gray-700">
                </div>
            </div>
        </div>
    </div>

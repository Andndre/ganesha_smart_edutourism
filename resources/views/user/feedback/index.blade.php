@extends('layouts.app')
@section('title', __('Riwayat Penilaian Saya'))
@section('header_title', __('Riwayat Penilaian'))
...
<div class="mx-auto max-w-6xl px-5 py-6">
    @if ($feedbacks->isEmpty())
        ...
        <div class="mb-6 text-center">
            ...
            <h3 class="text-charcoal mb-2 text-xl font-bold">{{ __('Belum Ada Penilaian') }}</h3>
            <p class="mb-6 text-sm text-gray-500">{{ __('Anda belum memberikan penilaian atau ulasan apapun.') }}</p>
            <a href="{{ route('feedback') }}"
                ...
                {{ __('Beri Penilaian Sekarang') }}
            </a>
        </div>
    @else
        {{-- Header: Count + New Feedback Button --}}
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-gray-500">{{ $feedbacks->count() }} {{ __('ulasan') }}</p>
            <a href="{{ route('feedback') }}"
            ...
            @foreach ($feedbacks as $fb)
                <a href="{{ route('feedback.show', $fb) }}"
                    ...
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <span class="text-xs text-gray-500">{{ $fb->created_at->translatedFormat('d M Y') }}</span>
                    </div>

    ...
    <form id="feedback-form" action="{{ route('feedback.update', $feedback) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6 flex flex-col items-center rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div id="rating-text" class="text-accent mt-2 h-5 text-sm font-bold opacity-100 transition-opacity">
                @php
                    $labels = ['Sangat Buruk', 'Kurang Memuaskan', 'Cukup Baik', 'Memuaskan', 'Sangat Memuaskan!'];
                @endphp
                {{ $labels[$feedback->rating - 1] ?? 'Sangat Memuaskan!' }}
            </div>
        </div>

        <!-- Comment -->
        <div class="mb-6">
            <label class="text-charcoal mb-2 block text-sm font-bold">Ulasan (Opsional)</label>
        ...
        @if ($feedback->photos && count($feedback->photos) > 0)
            <div class="mb-6">
                <label class="text-charcoal mb-2 block text-sm font-bold">Foto Saat Ini</label>
...
<script>
    (function() {
        ...
        const labels = ["Sangat Buruk", "Kurang Memuaskan", "Cukup Baik", "Memuaskan", "Sangat Memuaskan!"];

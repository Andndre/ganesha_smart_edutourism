            ...
            <a href="{{ route('umkm-product', $umkm->id) }}"
               class="relative flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all hover:border-gray-200 hover:shadow-md active:scale-[0.98]">
                <div class="relative aspect-square bg-gray-100">
                    ...
                    {{-- Rating Badge --}}
                    @if($umkm->rating)
                        <div class="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-0.5 text-xs font-bold text-amber-600 backdrop-blur-sm shadow-sm">
                ...
                <div class="flex-1 p-3">
                    <h3 class="text-charcoal text-sm font-bold line-clamp-1">{{ $umkm->business_name }}</h3>
                    @if($umkm->activeProducts->isNotEmpty())
                        <p class="mt-1 text-xs text-gray-400">{{ $umkm->activeProducts->first()->category?->name ?? '' }}</p>
                    @else

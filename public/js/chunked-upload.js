...
class ChunkedUploader {
    constructor(opts) {
        ...
        this.progressBar =
            opts.progressContainer?.querySelector(".tus-progress-bar");
        this.progressText =
            opts.progressContainer?.querySelector(".tus-progress-text");
        this.statusIcon =
        ...
        this.submitBtn =
            opts.submitButton || form?.querySelector('[type="submit"]');
        ...
        if (form) {
            ...
            form.addEventListener("submit", (e) => {
                const anyUploading = form._tusUploaders.some(
                    (u) => u.state === "uploading",
                );
                if (anyUploading) {
                    e.preventDefault();
    ...
    onFileSelect() {
        ...
        if (
            this.opts.allowedExtensions &&
            !this.opts.allowedExtensions.includes(ext)
        ) {
            this.showError(
                "Tipe file tidak diizinkan. Gunakan: " +
                    this.opts.allowedExtensions.join(", "),
            );
    ...
    showComplete(filename) {
        ...
        if (this.statusIcon) {
            this.statusIcon.innerHTML =
                '<svg class="h-4 w-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>';
        }
